...
Write-Host "Run all SOTA/canonical meta-learning pilot rows"
...
if (!$Execute) {
    Write-Host "Preview next pilot row only:"
    ...
    Write-Host ""
    Write-Host "Preview only. Add -Execute to run pilot rows sequentially."
    return
}
...
if (!$AllowConcurrent) {
    ...
    if ($activeStage2Python.Count -gt 0) {
        Write-Host "Active Stage 2 Python process detected; not starting all-row SOTA/canonical meta-learning pilot."
        $activeStage2Python |
...
for ($cycle = 1; $cycle -le $MaxCycles; $cycle++) {
    Write-Host ("=" * 80)
    Write-Host "SOTA/canonical meta-learning pilot cycle $cycle / $MaxCycles"

    $remaining = Get-RemainingRows $ManifestCsv
    if ($remaining.Count -eq 0) {
        Write-Host "All SOTA/canonical meta-learning pilot rows have checkpoints and evaluation CSVs."
        break
    }
    ...
    powershell @argsList
    if ($LASTEXITCODE -ne 0) {
        throw "SOTA/canonical meta-learning pilot cycle failed at cycle $cycle."
    }

    $remainingAfter = Get-RemainingRows $ManifestCsv
    if ($remainingAfter.Count -ge $remaining.Count -and !$DryRun) {
        throw "SOTA/canonical meta-learning pilot cycle made no progress at cycle $cycle. Remaining rows before=$($remaining.Count), after=$($remainingAfter.Count)."
    }
}

Write-Host ("=" * 80)
Write-Host "Finalizing SOTA/canonical meta-learning pilot outputs"
powershell -ExecutionPolicy Bypass -File scripts\stage2_finalize_meta_learning_outputs.ps1 `
    -ManifestCsv $ManifestCsv `
    -OutputDir $OutputDir
if ($LASTEXITCODE -ne 0) {
    throw "Pilot finalization failed."
}

Write-Host ("=" * 80)
Write-Host "Checking SOTA/canonical meta-learning pilot completion gate"
powershell -ExecutionPolicy Bypass -File scripts\stage2_sota_meta_learning_pilot_completion_gate.ps1 `
    -ManifestCsv $ManifestCsv `
    -OutputDir $OutputDir
if ($LASTEXITCODE -ne 0) {
    throw "SOTA/canonical meta-learning pilot completion gate failed."
}
