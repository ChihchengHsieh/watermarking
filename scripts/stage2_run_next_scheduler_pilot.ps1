...
if (!$AllowConcurrent) {
    ...
    if ($activeStage2Python.Count -gt 0) {
        Write-Host "Active Stage 2 Python process detected; not starting pilot job."
        $activeStage2Python |
...
$rows = Import-Csv $ManifestCsv |
    ...
    Sort-Object @{ Expression = { $priority[$_.scheduler] } }
...
if (!$selected) {
    Write-Host "Pilot scheduler queue is complete."
    Write-Host "Finalize:"
    Write-Host "powershell -ExecutionPolicy Bypass -File scripts\stage2_finalize_scheduler_outputs.ps1 -ManifestCsv $ManifestCsv -OutputDir $OutputDir"
    return
}
...
$evalCmd = "powershell -ExecutionPolicy Bypass -File scripts\eval_stage2_scheduler_run.ps1 -ManifestCsv $ManifestCsv -RunId $($selected.run_id) -BatchSize $BatchSize -TestingTimes $TestingTimes"
$finalizeCmd = "powershell -ExecutionPolicy Bypass -File scripts\stage2_finalize_scheduler_outputs.ps1 -ManifestCsv $ManifestCsv -OutputDir $OutputDir"
if ($DryRun) {
    $trainCmd += " -DryRun"
    $evalCmd += " -DryRun"
}
...
if (!$Execute) {
    Write-Host "Next pilot scheduler step: $($selected.run_id)"
...
if ($mode -eq "train_eval") {
    Write-Host ("=" * 80)
    Write-Host "Training pilot row: $($selected.run_id)"
    if ($DryRun) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Pilot training failed: $($selected.run_id)"
    }
} else {
...
Write-Host ("=" * 80)
Write-Host "Evaluating pilot row: $($selected.run_id)"
if ($DryRun -and !(Test-Path $selected.checkpoint_path)) {
    ...
    powershell -ExecutionPolicy Bypass -File scripts\eval_stage2_scheduler_run.ps1 -ManifestCsv $ManifestCsv -RunId $selected.run_id -BatchSize $BatchSize -TestingTimes $TestingTimes -DryRun
} else {
    powershell -ExecutionPolicy Bypass -File scripts\eval_stage2_scheduler_run.ps1 -ManifestCsv $ManifestCsv -RunId $selected.run_id -BatchSize $BatchSize -TestingTimes $TestingTimes
}
if (!$DryRun -and $LASTEXITCODE -ne 0) {
    throw "Pilot evaluation failed: $($selected.run_id)"
}

Write-Host ("=" * 80)
Write-Host "Finalizing pilot scheduler outputs"
powershell -ExecutionPolicy Bypass -File scripts\stage2_finalize_scheduler_outputs.ps1 -ManifestCsv $ManifestCsv -OutputDir $OutputDir
if ($LASTEXITCODE -ne 0) {
    throw "Pilot finalize failed after run: $($selected.run_id)"
}
