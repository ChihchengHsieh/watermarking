"""
This script defines an InversableStableDiffusionPipeline class, which extends a modified Stable Diffusion pipeline to support both forward and backward diffusion processes. The main features include:

DDIM Forward/Backward Functions:
    Implements functions to move between noise and image space using DDIM (Denoising Diffusion Implicit Models) equations.
Text Embedding:
    Provides a method to get text embeddings from prompts using a tokenizer and text encoder.
Latent Handling:
    Methods to encode images into latents, generate random latents, and decode latents back to images.
Backward Diffusion:
    The core method (backward_diffusion) generates images from text and latents, supporting prompt-to-prompt editing and classifier-free guidance.
Torch to Numpy Conversion:
    Utility to convert PyTorch image tensors to NumPy arrays for further processing or visualization.
The pipeline is designed for flexible image generation and inversion tasks using Stable Diffusion models.
"""
...
class InversableStableDiffusionPipeline(ModifiedStableDiffusionPipeline):
    def __init__(
        ...
        safety_checker,
        feature_extractor,
        image_encoder=None,  # Adding this argument for latest version of diffusers
